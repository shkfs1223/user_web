...
export default {
  ...
  methods: {
    setFileList() {
      if (this.value) {
        this.fileList = (Array.isArray(this.value) ? this.value : [this.value]).map((item, index) => {
          if (typeof item === 'string') {
            item = { fileName: item, url: item }
          }
          if (!item.uid) {
            item.uid = item.uid || new Date().getTime() + index
          }
          return item
        })
      } else {
    ...
    handleUploadSuccess(res, file) {
      ...
      if (this.limit > 1) {
        ...
        this.$emit('input', this.getBaseUrl() + fileObj.url)
      }
    },
    // 删除文件
    handleDelete(index) {
